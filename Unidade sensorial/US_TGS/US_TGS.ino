...
GxEPD_Class display(io, /*RST=D4*/ 12, /*BUSY=D2*/ 10); // default selection of D4(=2), D2(=4)

// ssid e passaword da Rede Aberta
const char* ssid = "MEO-37E7A0";                   // wifi ssid
const char* password =  "0b5240c704";         // wifi password

//Dados do mqtt
const char* mqttServer = "192.168.1.80";
...
bool verifytls() {
  ...
  Serial.println("10.3.141.1");
#endif

  //success = espClient.connect("192.168.1.80", 8883);
  success = espClient.connect("10.3.141.1", 8883);
...
void loop() {
  ...
  //Retirar os valores do sensor TGS
  float CO = analogRead(A0)*0.27;
  ...
  display.setFont(f);
  display.print("Monoxido de Carbono:");
  display.setFont(f2);
  ...
  //Publish
  String C = (String)CO;
  mqttClient.publish("TGS_C", C.c_str());
  mqttClient.subscribe("TGS_C");

  //Update apenas de alterações do display
